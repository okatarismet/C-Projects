...
int main (int argc, char **argv) {
    ...
    /* DECLARATIONS */
    struct type hero[100]; /* initialize to standard values for error-preventation */
    int m1 = 0;
    for (m1 = 0; m1 < 100; ++m1) {
        hero[m1].char_XP = 0;
        hero[m1].char_DP = 0;
    ...
    struct type monster[100]; /* initialize to standard values for error-preventation */
    int m = 0;
    for (m = 0; m < 100; ++m) {
        monster[m].char_XP = 0;
        monster[m].char_DP = 0;
 ...
 char **MAP;
    ...
    while ((char_readline_length = getline(&command_readline, &command_len, coptr)) != -1) { /*that reads line by line from commands file and stop when EOF*/
        ...
        if (strcmp(command_readline_split[0], "LOADMAP") == 0) {
            ...
            for (int l = 0; l < load_row; ++l) { /*set map to "."*/
