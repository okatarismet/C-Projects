...
int main(int argc, char *argv[]) { 
    ...
    std::ifstream input(argv[1]);                  // open the input.txt
  ...
  while(count < lenOfLinesArray){
    ...
    if(mostScoredHalf > 0)                  // print due to the counter data
        fprintf(fp,"1\n");
    else  if(mostScoredHalf < 0)  
        fprintf(fp,"0\n");
    else    
        fprintf(fp,"First half and the second half is equal!\n");
   ...
   fprintf(fp,"2)GOAL SCORER\n");
    ...
    for(int o = 0; o+1 < totalGamer ;o++){
        if(goalArray[o] > goalArray[mostGoaler]){
            mostGoaler = o;
        }
    }
    if(goalArray[totalGamer-1] > goalArray[mostGoaler])
            mostGoaler = totalGamer-1;
...
for(int uu=0; uu<2; uu++){
     for(size_t i = 0; i < totalGamer; i++){
        ...
        if(strcmp(curLine[uu], getnode(i)->futbolName) == 0){
            nodePtr cn = getnode(i);
            int curMacId = -1;
            while (head->next != NULL) // so we can get the last nodes pointer.
                head = head->next; 
            while (head != NULL) {  // and come with prev pointers.
                if(curMacId != head->macId){
                    if(uu== 0 || head->prev != NULL)
                fprintf(fp,"footballer Name: %s,Match ID: %d\n",cn->futbolName,head->macId);
                    else 
                fprintf(fp,"footballer Name: %s,Match ID: %d",cn->futbolName,head->macId);
