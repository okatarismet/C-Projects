...
int main (int argc, char **argv) {
    ...
    int mc = 0, hc = 0;
      ...
      if ((aptr = fopen(argv[1], "r")) == NULL) {
        printf("Error! opening file");
        exit(1);
    }
    char *char_readline1 = NULL;
    char char_readline_split1[4][25];
    size_t chars_len1 = 0;
    size_t char_readline_length1;
    while ((char_readline_length1 = getline(&char_readline1, &chars_len1, aptr)) != -1) {
        char *token1 = NULL;
        int n_tokens1 = 0, i1 = 0;
        token1 = strtok(char_readline1, ",");
        while (token1 != NULL) {  /*splitting block*/
            strcpy(char_readline_split1[i1], token1);
            token1 = strtok(NULL, ",");
            n_tokens1++;
            i1++;
        }
        if (strcmp(char_readline_split1[0], "HERO") == 0) { hc++; }
        if (strcmp(char_readline_split1[0], "MONSTER") == 0) { mc++; }
    }
    fclose(aptr);
    struct type *hero =  malloc(sizeof(struct type)*hc);
    int m1 = 0;
    for ( m1 = 0; m1 < hc; ++m1) {    /* initialize to standard values for error-preventation */
    ...
    struct type *monster = malloc(sizeof(struct type)*mc);
    int m = 0;
    for ( m = 0; m < mc; ++m) {    /* initialize to standard values for error-preventation */
 ...
 char **MAP;
    ...
    while ((char_readline_length = getline(&command_readline, &command_len, coptr)) != -1) { /*that reads line by line from commands file and stop when EOF*/
        ...
        if (strcmp(command_readline_split[0], "ATTACK") == 0) {
            if (strcmp(command_readline_split[1], "HERO\n") == 0) {
                fprintf(wptr, "%s", "HEROES ATTACKED\n");
            ...
            if (strcmp(command_readline_split[1], "MONSTER\n") == 0) {
                fprintf(wptr, "%s", "MONSTERS ATTACKED\n");
                        ...
                        if (command_readline_split[2 + 3 * x] != 0) {
                            ...
                            if (strcmp(command_readline_split[a], hero[z].char_name) == 0) {
                                if (hero[z].char_life == 0) {
                                    ...
                                    fprintf(wptr, "%s can’t move. There is a wall.\n", hero[z].char_name);
        ...
        int result = 0;
        int m;
        for (m = 0; m < monster_count; ++m) { /*if one of the heroes is alive the result must be different from zero*/
            result += (monster[m].char_life == 1);
        }
        if (result == 0) {
            fprintf(wptr, "%s", "ALL MONSTERS ARE DEAD!\n");
            printf("ALL MONSTERS ARE DEAD!\n");
            return 0;
        }
        result = 0;
        int n;
        for (n = 0; n < hero_count; ++n) {
            result += ((hero[n].char_life == 1));
        }
        if (result == 0) {
            fprintf(wptr, "%s", "ALL HEROES ARE DEAD! \n");
